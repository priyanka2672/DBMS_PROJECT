...
<html lang="en">
  ...
  <ul>
          ...
          <li><a  href="artistsearch.php">Search By Artist</a></li>
          <li><a class="active" href="eventsearch.php">Search By Event</a></li>
        ...
        <form method="POST" action="">
            <label>Enter Event ID of the Event to be Searched</label>
            <input type="number" class="form-control" name="event_id" placeholder="Event ID">
      ...
      <table class="table table-hover table-dark mytable">
        ...
        <?php
              ...
              {
              $name = $_POST['event_id'];
              $sql = "CALL eventsearch($name) ";
              $res = $conn->query($sql);

              if($res->num_rows >0){
                while($row = $res-> fetch_assoc()){
                  echo "<tr><td>". $row["Event_ID"]."</td><td>". $row["Article_title"]."</td><td>". $row["Article_style"]. "</td><td>". $row["Price"]. "</td><td>". $row["Date"]. "</td><td>". $row["Time"]. "</td><td>";
                }
                echo "</table>";
              }
              else{
